...
public class EgovCalRestdeManageController {
    ...
    /**
	 * 휴일을 등록한다. (cmd 미지정: 등록 폼, cmd=Regist: 등록 처리)
     * @param loginVO
     * @param restde
     * @param bindingResult
     * @param commandMap
     * @param model
     * @return "/cmm/sym/cal/EgovRestdeRegist"
     * @throws Exception
     */
	...
	public String insertRestde (@ModelAttribute("loginVO") LoginVO loginVO
		...
		if (sCmd.equals("Regist")) {
			boolean valid = restde.getRestdeDe() != null && restde.getRestdeDe().trim().length() == 8
					&& restde.getRestdeNm() != null && !restde.getRestdeNm().trim().equals("");

			if (bindingResult.hasErrors() || !valid) {
				model.addAttribute("restdeCode", cmmUseService.selectCmmCodeDetail(codeVO));
				model.addAttribute("message", "휴일일자(YYYYMMDD 8자리)와 휴일명은 필수 입력값입니다.");
				return "cmm/sym/cal/EgovRestdeRegist";
			}

			LoginVO user = (LoginVO) EgovUserDetailsHelper.getAuthenticatedUser();
			restde.setFrstRegisterId(user == null ? "" : user.getUniqId());
	...
	, ModelMap model) throws Exception {
		...
		if (sCmd.equals("")) {
    		...
    		LoginVO user = (LoginVO) EgovUserDetailsHelper.getAuthenticatedUser();
    		restde.setLastUpdusrId(user == null ? "" : user.getUniqId());
