...
import egovframework.com.cmm.RenewPaginationRenderer;
...
public class EgovConfigAppCommon {
	...
	/**
	 * @return [RenewPaginationRenderer 설정] renewRenderer 등록 (포털 게시판 페이징)
	 */
	@Bean
	...
	/**
	 * @return [PaginationManager 설정] image/renew 렌더러 등록.
	 */
	@Bean
	...
	/**
	 * 환경 암복호화 서비스 — EgovPasswordResolver(DB 패스워드 평문/암호 판별)가 참조.
	 * 개발 환경 DB 패스워드는 평문이므로 복호화는 실제로 수행되지 않는다.
	 * @return [EgovEnvCryptoServiceImpl 설정] egovEnvCryptoService 등록
	 */
	@Bean
	public EgovEnvCryptoServiceImpl egovEnvCryptoService() {
		EgovEnvCryptoServiceImpl egovEnvCryptoServiceImpl = new EgovEnvCryptoServiceImpl();
		egovEnvCryptoServiceImpl.setPasswordEncoder(egovPasswordEncoder());
		egovEnvCryptoServiceImpl.setCryptoService(egovARIACryptoService());
		return egovEnvCryptoServiceImpl;
	}
}
